 ...
 * Button.types.ts의 as const 배열과 JSDoc을 파싱해 마크다운 문서 생성
 */
import fs from 'fs';
...
interface ComponentDocConfig {
  ...
  variants?: string[];
  sizes?: string[];
  variantDescriptions?: Record<string, { background: string; content: string; description: string }>;
  sizeSpecs?: Record<string, { height: string; paddingX: string; typography: string }>;
  props: PropDef[];
...
export function generateButtonDoc(): string {
...
export function generateChipDoc(): string {
  return `# Chip
...
export function generateTextDoc(): string {
  return `# Text
...
export function generateIconDoc(): string {
  return `# Icon
...
function generateComponentMarkdown(config: ComponentDocConfig): string {
  const lines: string[] = [
    `# ${config.name}`,
    '',
    config.description,
