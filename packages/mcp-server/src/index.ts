...
import { getButtonUsage, getTextUsage, getIconUsage } from './tools/components.js';
import { getToken } from './tools/tokens.js';
import { searchDesignSystem } from './tools/search.js';
import { getSemanticToken } from './tools/semantic-token.js';
import { getDsCandidates, registerDsCandidate } from './tools/candidates.js';
import { discoverTools } from './tools/discovery.js';
import { listComponents, getComponent } from './tools/components-v2.js';
import { getDesignCheckStatus, getDesignCheckMismatches } from './tools/design-check.js';
import { listIcons, searchIcons } from './tools/icons.js';
  ...
  return {
    tools: [
      ...
      // Tokens
      {
        name: 'get_token',
        description:
          'MyRealTrip 디자인 토큰 조회. 색상(color), 그림자(shadow), 타이포그래피(typography) 토큰 값 반환.',
        inputSchema: {
          type: 'object',
          properties: {
            name: {
      ...
      },
      {
        ...
        description:
          '의미(intent) 기반으로 semantic/component 토큰 조회. 직접 hex 사용 전 반드시 확인.',
        inputSchema: {
          type: 'object',
          properties: {
            intent: {
      ...
      // Search
      {
        ...
        description:
          'BM25 알고리즘으로 디자인 시스템 전체를 검색. 자연어 쿼리 가능.',
        inputSchema: {
          type: 'object',
          properties: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'discover_tools': {
      return {
        content: [{ type: 'text', text: discoverTools(args?.query as string | undefined, args?.category as string | undefined) }],
      };
    }
    ...
    case 'list_components': {
      return {
        content: [{ type: 'text', text: listComponents(args?.platform as string | undefined) }],
      };
    }

    case 'get_component': {
      return {
        content: [{ type: 'text', text: getComponent(args?.name as string, args?.platform as string | undefined) }],
      };
    }
    ...
    case 'get_token': {
      return {
        content: [{ type: 'text', text: getToken(args?.name as string) }],
      };
    }

    case 'get_semantic_token': {
      return {
        content: [{ type: 'text', text: getSemanticToken(args?.intent as string) }],
      };
    }
    ...
    case 'search_design_system': {
      return {
        content: [{ type: 'text', text: searchDesignSystem(args?.query as string) }],
      };
    }
    ...
    case 'get_design_check_status': {
      return {
        content: [{ type: 'text', text: getDesignCheckStatus() }],
      };
    }

    case 'get_design_check_mismatches': {
      return {
        content: [{ type: 'text', text: getDesignCheckMismatches(args?.component as string, args?.platform as string | undefined) }],
      };
    }
    ...
    case 'list_icons': {
      return {
        content: [{ type: 'text', text: listIcons(args?.category as string | undefined) }],
      };
    }

    case 'search_icons': {
      return {
        content: [{ type: 'text', text: searchIcons(args?.keyword as string) }],
      };
    }
    ...
    case 'get_ds_candidates': {
      return {
        content: [{ type: 'text', text: getDsCandidates(args?.status as string | undefined) }],
      };
    }
    ...
    case 'get_button_usage': {
      ...
      const platform = (args?.platform as string | undefined) ?? 'web';
      return {
        content: [{ type: 'text', text: getButtonUsage(variant, useCase, platform) }],
      };
    }
    ...
    case 'get_text_usage': {
      ...
      const platform = (args?.platform as string | undefined) ?? 'web';
      return {
        content: [{ type: 'text', text: getTextUsage(useCase, platform) }],
      };
    }
    ...
    case 'get_icon_usage': {
      ...
      const platform = (args?.platform as string | undefined) ?? 'web';
      return {
        content: [{ type: 'text', text: getIconUsage(keyword, platform) }],
      };
    }
...
→ sources: ${sourceFiles ? `["${sourceFiles.split(',').map((s: string) => s.trim()).join('", "')}"]` : '현재 작업 파일 경로'}
