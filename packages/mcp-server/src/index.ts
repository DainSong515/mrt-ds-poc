  ...
  return {
    tools: [
      // 기존 4개 tool
      {
        ...
        description:
          'MyRealTrip Button 컴포넌트 사용법 반환. variant별 Props, 예시 코드, 주의사항 포함. 버튼 구현 시 반드시 호출.',
        inputSchema: {
          type: 'object',
          properties: {
            variant: {
              type: 'string',
              description:
                "조회할 variant. primary | primaryOpt | secondary | secondaryOpt | tertiary | tertiaryOpt | text. 생략 시 전체 반환.",
            },
            use_case: {
              type: 'string',
      ...
      },
      {
        name: 'get_text_usage',
        description:
          'MyRealTrip Text 컴포넌트 사용법 반환. typography 전체 목록, lineClamp 사용법 포함. 텍스트 구현 시 반드시 호출.',
        inputSchema: {
          type: 'object',
          properties: {
            use_case: {
      ...
      },
      {
        name: 'get_icon_usage',
        description:
          'MyRealTrip Icon 컴포넌트 사용법 및 아이콘 검색. 아이콘 사용 시 반드시 호출. Button의 leftIcon/rightIcon prop에 전달할 아이콘 이름 확인.',
        inputSchema: {
          type: 'object',
          properties: {
            keyword: {
              type: 'string',
              description:
                "검색 키워드 (한글/영문). 예: '뒤로', '검색', 'arrow', 'search'. 생략 시 카테고리별 전체 목록 반환.",
            },
          },
        },
      },
      {
        name: 'get_token',
        description:
          'MyRealTrip 디자인 토큰 조회. 색상(color), 그림자(shadow), 타이포그래피(typography) 토큰 값 반환. semantic/component 토큰도 포함.',
        inputSchema: {
          type: 'object',
          properties: {
            name: {
              type: 'string',
              description:
                "토큰 이름 또는 키워드. 예: 'gray', 'blue.500', 'shadow.E200', 'headlineBold20', 'surface', 'button.primary'",
            },
          },
          required: ['name'],
        },
      ...
      // 신규 5개 tool
      {
        ...
        description:
          'BM25 알고리즘으로 디자인 시스템 전체를 검색. 컴포넌트명/기능/사용 맥락으로 관련 문서 스니펫 반환. "로딩 버튼", "텍스트 말줄임" 같은 자연어 쿼리 가능.',
        inputSchema: {
          type: 'object',
          properties: {
            query: {
      ...
      },
      {
        ...
        description:
          '의미(intent) 기반으로 semantic/component 토큰 조회. "surface", "disabled", "CTA" 같은 의도로 검색. 직접 hex 사용 전 반드시 확인.',
        inputSchema: {
          type: 'object',
          properties: {
            intent: {
      ...
      },
      {
        ...
        description:
          '디자인 시스템 후보군 목록 반환. frontend-dubai에서 DS로 통합 예정인 컴포넌트 목록. 유사 컴포넌트 반복 구현 전 확인 권장.',
        inputSchema: {
          type: 'object',
          properties: {
            status: {
      ...
      },
      {
        ...
        description:
          'DS 후보군에 새 컴포넌트 등록. 유사 컴포넌트가 반복 구현된다고 판단될 때 호출. pending 상태로 등록되며 디자이너 협의 후 편입.',
        inputSchema: {
          type: 'object',
          properties: {
            id: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    case 'get_button_usage': {
      ...
      return {
        content: [{ type: 'text', text: getButtonUsage(variant, useCase) }],
      };
    }
    ...
    case 'get_text_usage': {
      const useCase = args?.use_case as string | undefined;
      return {
        content: [{ type: 'text', text: getTextUsage(useCase) }],
      };
    }
    ...
    case 'get_icon_usage': {
      ...
      return {
        content: [{ type: 'text', text: getIconUsage(keyword) }],
      };
    }
    ...
    case 'get_token': {
      ...
      return {
        content: [{ type: 'text', text: getToken(tokenName) }],
      };
    }
    ...
    case 'search_design_system': {
      ...
      return {
        content: [{ type: 'text', text: searchDesignSystem(query) }],
      };
    }
    ...
    case 'get_semantic_token': {
      ...
      return {
        content: [{ type: 'text', text: getSemanticToken(intent) }],
      };
    }
    ...
    case 'get_ds_candidates': {
      ...
      return {
        content: [{ type: 'text', text: getDsCandidates(status) }],
      };
    }
