...
const REGISTRY_PATH = path.resolve(__dirname, '../../../../data/candidate-registry.json');

export type CandidateStatus = 'pending' | 'in-progress' | 'completed' | 'rejected';
...
function loadRegistry(): DSCandidate[] {
  try {
    return JSON.parse(fs.readFileSync(REGISTRY_PATH, 'utf-8')) as DSCandidate[];
  } catch {
    return [];
  }
}

function saveRegistry(registry: DSCandidate[]): void {
  fs.writeFileSync(REGISTRY_PATH, JSON.stringify(registry, null, 2), 'utf-8');
}

export function getDsCandidates(status?: string): string {
  ...
  const statusEmoji: Record<CandidateStatus, string> = {
    pending: '⏳',
    'in-progress': '🔨',
    completed: '✅',
    rejected: '❌',
...
export function registerDsCandidate(params: {
  id: string;
  description: string;
  sources: string[];
  similarity?: number;
  ...
  const newCandidate: DSCandidate = {
    id: params.id,
    status: 'pending',
