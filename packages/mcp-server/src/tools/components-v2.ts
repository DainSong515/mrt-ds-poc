...
import { getButtonUsage, getTextUsage, getIconUsage } from './components.js';
...
interface ComponentInfo {
  ...
  platforms: {
    ...
    ios: { available: boolean; framework: string };
  };
}
...
function loadMatchRates(): Record<string, Record<string, number>> {
  try {
    ...
    for (const p of candidates) {
      if (fs.existsSync(p)) {
        ...
        for (const r of data.results) {
          if (!rates[r.component]) rates[r.component] = {};
...
export function listComponents(platform?: string): string {
  ...
  for (const comp of COMPONENT_REGISTRY) {
    if (platform && platform === 'ios' && !comp.platforms.ios.available) continue;
    ...
    lines.push(
      `| ${comp.name} | ${comp.webComponent} | ${comp.iosComponent} | ${comp.iosFramework} | ${rateStr} |`,
    );
  }
...
export function getComponent(name: string, platform?: string): string {
  ...
  switch (normalized) {
    case 'button':
      return getButtonUsage(undefined, undefined, p);
    case 'text':
      return getTextUsage(undefined, p);
    case 'icon':
      return getIconUsage(undefined, p);
    case 'chip':
...
function getChipSpec(platform: string): string {
  if (platform === 'ios') {
...
function getTabBarSpec(platform: string): string {
  if (platform === 'ios') {
